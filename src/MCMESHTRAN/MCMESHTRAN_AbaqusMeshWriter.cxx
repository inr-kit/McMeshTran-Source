...
bool     MCMESHTRAN_AbaqusMeshWriter::exportFile (const QString & OutFileName)
{
    ...
    for (int i=0; i<myMeshList.size(); i++)
    {
        ...
        set <INTERP_KERNEL::NormalizedCellType> ElmTypes = aMesh->getAllGeoTypes(); //qiu 28-08-14
        if (ElmTypes.size() > 2) {
        ...
        QString aTmpName;
        if (aMesh->getName().c_str() == 0) //if no name//qiu 28-08-14
            aTmpName = QString("Mesh-") + QString("%1").arg(i+1);
        else
            aTmpName = aMesh->getName().c_str();//qiu 28-08-14
